...
using KGJ.Authorization.Accounts;
...
namespace KGJ.Users
{
    ...
    public class UserAppService : AsyncCrudAppService<User, UserDto, long, PagedUserResultRequestDto, CreateUserDto, UserDto>, IUserAppService
    {
        ...
        public override async Task<UserDto> CreateAsync(CreateUserDto input)
        {
            ...
            await _userManager.SetOrganizationUnitsAsync(user.Id, input.OrganizationUnits);
        ...
        public void TestPost(TestModel model)
        {

        }

        [HttpGet]
        public void TestGet(TestModel model)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<ListResultDto<OrganizationUnitTreeDto>> GetUserOrganizationUnitsAsync(long id)
        {
            //get user's organization
            var organizationUnitsIds = new List<long>();
            if (id > 0)
            {
                var user = await _userManager.GetUserByIdAsync(id);
                var organizationUnitsResult = await _userManager.GetOrganizationUnitsAsync(user);
                organizationUnitsIds = organizationUnitsResult.Select(ou => ou.Id).ToList();
            }
            ...
            {
                var treeDto = ConvertOrganizationUnitToTree(item, organizationUnitMemberCounts, organizationUnitsIds);
        ...
        private OrganizationUnitTreeDto ConvertOrganizationUnitToTree(OrganizationUnit item, Dictionary<long, int> memberCounts, List<long> organizationUnitsIds)
        {
            ...
            if (organizationUnitsIds.Exists(p => p == item.Id))
            {
                organizationUnitTreeDto.Checked = true;
            }
            ...
            {
                var childTree = ConvertOrganizationUnitToTree(itemChild, memberCounts, organizationUnitsIds);
